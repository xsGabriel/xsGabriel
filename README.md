# Olá! 👋 Bem-vindo ao meu GitHub!

Sou um estudante universitário apaixonado por tecnologia, desenvolvimento de software e análise de sistemas. Gosto de atuar em todas as etapas de um projeto, desde o mapeamento de processos e stakeholders até a codificação do back-end e a estilização do front-end.

Tenho um interesse especial por **Desenvolvimento Full-Stack** e na construção de arquiteturas lógicas e banco de dados.

---

### 💻 Minhas Habilidades e Tecnologias

Aqui estão algumas das tecnologias e ferramentas com as quais trabalho e estudo:

**Back-end & Banco de Dados:**
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=java&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-404D59?style=for-the-badge)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![MySQL Workbench](https://img.shields.io/badge/MySQL_Workbench-005C84?style=for-the-badge&logo=mysql&logoColor=white)

**Front-end & Design:**
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

**Engenharia de Software & Processos:**
![BPMN](https://img.shields.io/badge/BPMN_2.0-F26522?style=for-the-badge)
![UML & Mapeamento](https://img.shields.io/badge/Análise_de_Sistemas-000000?style=for-the-badge)

---

### 🚀 Projetos em Destaque

Aqui estão alguns dos projetos em que venho trabalhando:

*   **FinancePro:** Um conversor de moedas desenvolvido em Python com integração de API em tempo real e histórico de taxas.
*   **VisionWeb:** Sistema de Pesquisas de Clima Organizacional, focado em regras de negócio para níveis de acesso de colaboradores e administradores.
*   **Catálogo de Relíquias Literárias:** Projeto web com foco em UI/UX, trabalhando a responsividade e alinhamento de cards em HTML/CSS.
*   **Mapeamento e Processos:** Experiência prática com diagramação BPMN 2.0 (ex: *ReverseLab*) e mapeamento de stakeholders em projetos de TI (ex: *Projeto Aster*).

---

### ⚡ Curiosidades sobre mim

- 📚 Nas horas vagas, gosto bastante de ler webnovels e mangás (especialmente *Shadow Slave* e *SSS-Class Suicide Hunter*).
- ⚽ Sou um torcedor apaixonado do São Paulo Futebol Clube (MorumBIS é a nossa casa!).
- 💡 Gosto de histórias e dinâmicas que envolvam contrastes, como o conceito de "luz e sombra".

---

### 📫 Como me encontrar

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](SEU_LINK_DO_LINKEDIN_AQUI)
[![E-mail](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:SEU_EMAIL_AQUI)

---
*📊 Estatísticas do GitHub*

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=SEU_USUARIO_AQUI&show_icons=true&theme=radical" alt="Estatísticas do GitHub" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=SEU_USUARIO_AQUI&layout=compact&theme=radical" alt="Linguagens mais usadas" />
</div>
